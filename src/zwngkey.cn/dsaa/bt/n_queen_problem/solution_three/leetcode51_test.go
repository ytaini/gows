 ...
 * @LastEditTime: 2022-11-02 14:06:01
 * @Description:
	方法3: 基于位运算的回溯
...
func totalNQueens(n int) (ans int) {
	...
	bt = func(row, cols, dia1, dia2 int) {
		if row == n { //当 N 个皇后都放置完毕，则找到一个可能的解，将可能的解的数量加 1。
			ans++
			return
		}
		v1 := cols | dia1 | dia2 //v1表示的二进制的数位的1表示不可以放置皇后的位置,例如: 0000000000000011
		v2 := ^v1                //对其取反,得到例如:1111111111111100,但是这样的话,1太多了.我们只需要n位的1,如八皇后,那么我们只需要最后的8位.
		v3 := (1<<n - 1)         //生成n个1的正数 如:0000000011111111
		bits := v2 & v3          //通过v2&v3操作,从v2中取出最后n位, 这样我们就从v2中取出了最后8位的情况.即0000000011111100,这样的话,这里的1表示可以放置皇后的位置
		for bits > 0 {           //如果bits中还存在1,说明还存在可以放置皇后的位置,通过回溯就可以测探当前行的每一种情况.
			// 每次获得可以放置皇后的位置中的最低位，并将该位的值置成0，尝试在该位置放置皇后。这样即可遍历每个可以放置皇后的位置。
			pick := bits & -bits                                 //这里的pick取出了bits中的最低位的1，表示尝试在该位置放置皇后
			bt(row+1, cols|pick, (dia1|pick)<<1, (dia2|pick)>>1) //递归每一行,寻找可以放置皇后的位置.
			bits &= bits - 1                                     //该位的值置成0
			//bits &^= pick 这两个操作等价
		}
	}
