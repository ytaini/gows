/*
 * @Author: wzmiiiiii
 * @Date: 2022-11-24 12:52:29
 * @LastEditors: wzmiiiiii
 * @LastEditTime: 2022-11-24 13:46:19
 * @Description:
 */
...
// 我们也不能让多个接收者之一关闭一个额外的信号通道。 这两种做法都违反了通道关闭原则。
...
func main() {
	...
	// 它的接收者为dataCh 数据通道的所有的发送者和接收者。
	stopCh := make(chan struct{})
	// toStop是一个用来通知中间调解者让其关闭信号通道stopCh的第二个信号通道。
	// 此第二个信号通道的发送者为dataCh数据通道的所有的发送者和接收者，
	...
	for i := 0; i < NumSenders; i++ {
		go func(id string) {
			for {
				...
				// 即使stopCh已关闭，如果这个select代码块 中第二个分支的发送操作是非阻塞的，
				// 则第一个分支仍很有可能在若干个循环步内依然不会被选中。
	...
	for i := 0; i < NumReceivers; i++ {
		go func(id string) {
			...
			for {
				// 和发送者协程一样,此处的尝试接收操作是为了让次接收协程尽早退出
				select {
				case <-stopCh:
					return
				default:
				...
				// 即使stopCh已关闭，如果这个select代码块中第二个分支的接收操作是非阻塞的，
				// 则第一个分支仍很有可能在若干个循环步内依然不会被选中。
